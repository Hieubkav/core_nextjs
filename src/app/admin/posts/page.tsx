...
import PostCreateModal from '../../../components/admin/PostCreateModal'
  ...
  return (
    <div>
                  ...
                  <button
                    ...
                    className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                      page === pagination.page
                        ? 'z-10 bg-blue-50 border-blue-500 text-blue-60'
      ...
      {showDeleteModal && deletingPost && (
        <DeleteConfirmModal
          title="Xóa bài viết"
          message={`Bạn có chắc chắn muốn xóa bài viết "${deletingPost.title}"?`}
          onClose={() => setShowDeleteModal(false)}
          onConfirm={async () => {
            try {
              const response = await fetch(`/api/admin/posts/${deletingPost.id}`, {
                method: 'DELETE'
              })
              if (response.ok) {
                handleDeleteSuccess()
